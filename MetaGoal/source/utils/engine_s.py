...
class Trainer(object):
    ...
    def train(self):
        ...
        # self.use_rl = True
        # self.valid_metric_name = "bleu_score"
    ...
    def train_epoch(self):
        ...
        for batch_idx in range(num_batches):
            ...
            '''
            if batch_idx == 0:
                print("max_turn:", local_data['max_turn'])
                print("len kbs:", len(local_data['kbs']))
                print("kbs:")
                for kb in local_data['kbs']:
                    print(self.src_field.denumericalize(kb))
                for i, inputs in enumerate(turn_inputs):
                    ...
                    print(src.size(), self.src_field.denumericalize(src))
                    print(src_lengths.size(), src_lengths)
                    print(tgt.size(), self.src_field.denumericalize(tgt))
                    print(tgt_lengths.size(), tgt_lengths)
                    print("ptr_index:", ptr_index.size(), ptr_index)
                    print("kb_index:", kb_index.size(), kb_index)
                    print("gold_entity:", inputs.gold_entity)
                #kbs, kb_lens = kb_inputs
                #for kb in kbs:
                #    print(kb)
                #    print('')
            '''
            metrics_list,batch_loss = self.model.iterate(turn_inputs, kb_inputs,
                                              optimizer=self.optimizer,
    ...
    def evaluate(model, data_iter, use_rl=False):
        ...
        with torch.no_grad():
            for batch_idx in range(num_batches):
                ...
                metrics_list,batch_loss = model.iterate(turn_inputs, kb_inputs,
