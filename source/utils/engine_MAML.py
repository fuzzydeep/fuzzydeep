...
class MetricsManager(object):
    ...
    def update(self, metrics_list):
        ...
        update
        """
        for i, metrics in enumerate(metrics_list[0]):
            # metrics.pop("logits")
            # metrics.pop("prob")
...
class Trainer(object):
    ...
    def get_data_loader(self, data_iter,is_traing=True,batch_idx=None):
        num_batches = data_iter.n_batch
        if is_traing:
            local_data = data_iter.get_unprepared_batch(random.randint(0,num_batches-1))
        else:
            local_data = data_iter.get_unprepared_batch(batch_idx)
        return local_data
    def train_MPL(self):
        self.epoch += 1
        ...
        for batch_idx in range(num_batches):
            ...
            self.model.learning_phrase_1(self.model.model_KB, turn_inputs, kb_inputs, optimizer=self.optimizer,
                                                 grad_clip=self.grad_clip, mode="kb")

            self.model.learning_phrase_1(self.model.model_Dialog, turn_inputs, kb_inputs, optimizer=self.optimizer,mode="dialog")
        ...
        for batch_idx in range(num_batches):
            ...
            self.model.learning_phrase_2(self.model.model_KB, turn_inputs, kb_inputs,
                                                 optimizer=self.optimizer,
                                                 grad_clip=self.grad_clip, mode="kb")
            # loss accumulate from a batch of tasks
            self.model.learning_phrase_2(self.model.model_Dialog, turn_inputs, kb_inputs,
                                                 optimizer=self.optimizer,mode="dialog")
    ...
    def train_MAML(self):
        ...
        for batch_idx in range(num_batches):
            ...
            # batch_loss = self.model.iterate(turn_inputs, kb_inputs,
            #                                      optimizer=self.optimizer,
            #                                      grad_clip=self.grad_clip,
            #                                      use_rl=self.use_rl,
            #                                      is_training=True)
            batch_loss = self.model.iterate(turn_inputs, kb_inputs, meta_optimizer=self.optimizer,
                                            grad_clip=self.grad_clip,is_training=True)
        ...
        # self.save()
        # # train_mm.update(metrics_list_S)
        # elapsed = time.time() - start_time
        # self.logger.info('')
        # if (batch_idx + 1) % self.log_steps == 0:
        #     message_prefix = "[Train][{:2d}][{}/{}]".format(self.epoch, batch_idx + 1, num_batches)
        ...
        # self.save()
        # best_loss = 10000000
        # for meta_iteration in range(self.epochs+1):
        #         #
        #         #     train_iter = self.get_data_loader(self.train_iter)
        #         #     val_iter = self.get_data_loader(self.train_iter)
        #         #     batch_loss = self.model.iterate(train_iter, val_iter, meta_optimizer = self.optimizer,grad_clip = self.grad_clip)
        #         #
        #         #     print('Meta_iteration:', meta_iteration)
        #         #     ## Meta-Evaluation 将参数放到meta-test set上进行测试，然后对测试集中的每个task做fine-tune�?
        #         #     if meta_iteration % 50 == 0:
        #         #         print('Meta_iteration:', meta_iteration)
        #         #         self.save()
        #         #         # val_loss_meta=[]
        ...
        #         #         # for batch_idx in range(num_batches):
        #         #         #     # mate tuning
        #         #         #     train_iter = self.get_data_loader(self.valid_iter,is_traing=False,batch_idx=batch_idx)
        #         #         #     val_iter = self.get_data_loader(self.valid_iter,is_traing=False,batch_idx=batch_idx)
        #         #         #     val_loss = self.model.iterate(train_iter, val_iter, meta_optimizer=self.optimizer)
        #         #         #     val_loss_meta.append(val_loss.item())
        #         #         #
        #         #         # print('Meta_loss:', np.mean(val_loss_meta))
        #         #         # # check early stop
    ...
    def train_epoch(self):
        ...
        for batch_idx in range(num_batches):
            ...
            metrics_list,_ = self.model.iterate(turn_inputs, kb_inputs,
                                              optimizer=self.optimizer,
                                              grad_clip=self.grad_clip,
                                              use_rl=self.use_rl,
                                              is_training=True)
    ...
    @staticmethod
    def evaluate(model, data_iter, use_rl=False):
        """
        evaluate
        """
        model.eval()
    ...
    def save(self, is_best=False, is_rl=False):
        ...
        train_state = {"epoch": self.epoch,
                       ...
                       "optimizer": self.optimizer['T'].state_dict()}
        if self.lr_scheduler is not None:
            train_state["lr_scheduler"] = self.lr_scheduler['T'].state_dict()
    ...
    def load(self, file_ckpt):
        ...
        if os.path.isfile(os.path.join(self.save_dir, file_ckpt)):
            ...
            self.optimizer['T'].load_state_dict(train_state_dict["optimizer"])
            if self.lr_scheduler is not None and "lr_scheduler" in train_state_dict:
                self.lr_scheduler['T'].load_state_dict(train_state_dict["lr_scheduler"])
            self.logger.info(
    ...
    def load_per_agent(self, S_ckpt, TE_ckpt, TB_ckpt):
        ...
        if os.path.isfile(os.path.join(self.pre_train_dir, S_ckpt)) and \
                ...
                ckpts = [S_ckpt, TB_ckpt, TE_ckpt]
                for file_ckpt in ckpts:
                    model_file = "{}/{}".format(self.pre_train_dir, file_ckpt)
                    model_state_dict = torch.load(model_file, map_location=lambda storage, loc: storage)
                    if file_ckpt == S_ckpt:
                        self.model.model_T.load_state_dict(model_state_dict)
                        self.logger.info("Loaded model state from '{}'".format(model_file))
                    elif file_ckpt == TB_ckpt:
                        self.model.model_Dialog.load_state_dict(model_state_dict)
                        self.logger.info("Loaded model state from '{}'".format(model_file))
                    elif file_ckpt == TE_ckpt:
                        if self.model.model_KB is not None:
                            assert os.path.isfile(os.path.join(self.pre_train_dir, TE_ckpt))
                            self.model.model_KB.load_state_dict(model_state_dict)
                            self.logger.info("Loaded model state from '{}'".format(model_file))
                    else:
